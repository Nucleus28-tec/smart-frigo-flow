...
| `finalize_journal_import(_file_id)` | admin | Fecha a importação: confere débito × crédito, compara contas e saldos com o período anterior (possível truncamento) e sinaliza encerramentos que entraram na base. Define `imported_files.processing_status` como `processado`, `processado_com_alertas` (lançamentos gravados, mas com divergência de conferência) ou `erro` (nada gravado), com o motivo. Retorna totais, `difference` e `warnings`. |
| `journal_import_diagnostics(_period_id)` | usuário logado | Relatório de inconformidades do razão do período: documentos em que débito ≠ crédito, contrapartidas citadas sem conta no período (perna ausente no arquivo do G2) e comparação do nº de contas com o período anterior (truncamento). Alimenta o painel e a exportação Excel em `/importar`. |
