...
- **Seções da tela:**
  ...
  - **Plano de contas › Árvore hierárquica — clique na conta:** clicar em qualquer linha da árvore abre à direita o **painel do razão daquela conta** (contas do ramo, no caso de grupo sintético) com busca, alternância "mostrar ocultos" e largura ajustável. No painel é possível **editar** o lançamento em linha (data, documento, valor, contas de débito/crédito e histórico), cancelar, comentar, **reclassificar a conta** (mover para outro grupo, com prévia de posição e de sinal) e **ocultar/reexibir a conta no resultado** (pede motivo e regera os demonstrativos). Contas ocultas aparecem riscadas na árvore. O checkbox de seleção em massa continua independente do clique; ações de escrita são restritas a Admin.
