...
3. Importar o **razão** em `/importar` (PDF, CSV, XLSX ou XLS). Em planilha, o sistema abre o **mapeamento de colunas**, pré-seleciona por semelhança de nome, converte valores em Real e datas, e mostra a pré-visualização com o resumo "X linhas válidas / Y com problema" antes de gravar. Por padrão os **lançamentos de encerramento do G2 são descartados** (opção "Descartar lançamentos de encerramento" na tela), porque o fechamento contábil passa a ser executado no RotaBase. Cada bloco é gravado de forma atômica: se algo falhar, nada entra, o arquivo fica com status **Erro** e o motivo aparece na lista. Ao final, o sistema valida débito × crédito e compara contas e saldos com o período anterior — divergência ou possível truncamento viram alerta explícito em vez de sucesso silencioso.
