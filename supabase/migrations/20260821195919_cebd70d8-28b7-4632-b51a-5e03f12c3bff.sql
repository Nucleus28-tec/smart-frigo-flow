...
begin
  ...
  with base as (
    select
      b.reduced_code,
      b.account_name,
      coalesce(la.hierarchical_code, b.hierarchical_code) as hier,
      b.nature,
      case
  ...
  synth as (
    ...
    where l.hier is not null and l.hier <> ''
  ...
  nodes as (
    ...
    left join public.ledger_accounts la
      on la.hierarchical_code = s.hier and coalesce(la.is_analytic, false) = false
    group by s.hier
    union all
    select
      ...
      case
        when l.hier is null or l.hier = '' then null
        else nullif(regexp_replace(l.hier, '\.[^.]+$', ''), l.hier)
      end as parent,
      true as is_analytic,
      l.reduced_code,
      array[l.reduced_code] as codes,
      l.valor
    from leaves l
  )
