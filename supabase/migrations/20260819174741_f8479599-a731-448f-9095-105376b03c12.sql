...
begin
  ...
  with grid as (
    select l.id, l.entry_group, l.status, l.origin, l.doc_number, l.entry_date, l.historico,
           ...
           coalesce(a.name, o.account_name, l.account_reduced_code) as debit_name,
           l.counterpart_reduced_code as credit_code,
           coalesce(c.name, co.account_name, l.counterpart_reduced_code) as credit_name,
           l.line_no
    ...
    left join public.journal_account_openings o
      on o.period_id = _period_id and o.account_reduced_code = l.account_reduced_code
    left join public.journal_account_openings co
      on co.period_id = _period_id and co.account_reduced_code = l.counterpart_reduced_code
    where l.period_id = _period_id
      ...
      and (
        ...
        or public.txt_norm(o.account_name) like '%' || q || '%'
        or public.txt_norm(co.account_name) like '%' || q || '%'
